...
| 模型 | 推荐语言 | 推荐理由 |
|------|---------|---------|
| `fire-red-asr-v1` | `zh` | 中文 ASR SOTA（CER ≈ 2%），追求极致精度可选；不加入 en 推荐（1.74 GB 体积过大，Whisper Turbo 更合适） |
| `paraformer-trilingual` | `zh` / `en` / `yue` | 真三语模型，粤语当前唯一可用；中英场景作为可选推荐 |
| `zipformer-ctc-zh` | `zh` | 中文专用离线 CTC，作为轻量备选与 SenseVoice / Paraformer 共存 |

> 三款模型均加入语言推荐池；用户选择对应语言时会看到候选项，再根据 description 中的优缺点权衡取舍。
...
- [x] 三款新模型均进入对应语言的推荐池（`test_paraformer_trilingual_recommended_languages` / `test_fire_red_asr_v1_recommended_for_chinese_only` / `test_zipformer_ctc_zh_recommended_for_chinese_only` 通过）
